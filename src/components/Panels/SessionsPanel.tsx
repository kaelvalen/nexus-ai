...
export function SessionsPanel({ onSectionChange }: Props) {
  ...
  const formatAge = (sessionId: string) => {
  ...
  const SessionRow = ({ s }: { s: ReturnType<typeof Object.values<typeof sessions[string]>>[0] }) => {
    const win = windows.find((w) => w.props.sessionId === s.id)
    ...
    return (
      <div className={`session-row ${s.active ? 'session-row-live' : 'session-row-dead'}`}>
        ...
        <div className="session-row-meta">
          ...
          <span className="session-meta-chip">{msgCount} MSGS</span>
        ...
        <div className="session-row-actions">
          {win && (
            <button className="row-action-btn" onClick={() => focusSession(s.id)} title="Focus window">
              <span className="mat mat-sm">open_in_full</span>
            </button>
          )}
          {s.active && (
            <button className="row-action-btn row-action-danger" onClick={() => handleKill(s.id)} title="Kill session">
              <span className="mat mat-sm">stop_circle</span>
            </button>
          )}
          {!s.active && (
            <button className="row-action-btn" onClick={() => removeSession(s.id)} title="Remove from list">
      ...
      <div className="section-body">
        ...
        {live.length > 0 && (
          <div className="panel-block">
            ...
            <div className="session-list">
              {live.map((s) => <SessionRow key={s.id} s={s} />)}
            </div>
          </div>
        )}
        ...
        {dead.length > 0 && (
          <div className="panel-block">
            ...
            <div className="session-list">
              {dead.map((s) => <SessionRow key={s.id} s={s} />)}
            </div>
          </div>
        )}
